...
when 'debian'
  package 'php5-memcached' do
    action :install
    notifies(:run, "execute[/usr/sbin/php5enmod memcached]", :immediately) if platform?('ubuntu') && node['platform_version'].to_f >= 12.04
  end
end

execute '/usr/sbin/php5enmod memcached' do
  action :nothing
